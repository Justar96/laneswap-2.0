...
def generate_mock_services(count: int = 10) -> None:
    ...
    for i in range(count):
        ...
        SERVICES[service_id] = {
            ...
            "name": service_name,
            "status": status,
            "created_at": (datetime.datetime.now() - datetime.timedelta(days=
                random.randint(1, 30))).isoformat(),
            "last_heartbeat": (datetime.datetime.now() - datetime.timedelta(minutes=
                random.randint(0, 60))).isoformat(),
    ...
    while True:
        for service_id, service in SERVICES.items():
            ...
            if random.random() < 0.1:
                ...
                logger.info("Updated service %s status to %s", service['name'], new_status)
...
def main():
    ...
    logger.info("Starting mock API server on %s:%s", args.host, args.port)
