...
def start_web_monitor(api_url, port=8080, open_browser=True):
    ...
    try:
        ...
        logger.info("Web monitor started on port %s", port)
        logger.info("You can access it at: http://localhost:%s", port)

        return True
    except Exception as e:
        logger.error("Failed to start web monitor: %s", str(e))
        return False

def start_server(host=
    "0.0.0.0", port=8000, debug=False, start_monitor=True, monitor_port=8080, open_browser=True):
    """
    ...
    logger.info("Starting LaneSwap API server on %s:%s", host, port)
    ...
    if start_monitor:
        ...
        if monitor_started:
            logger.info("Web monitor available at: http://localhost:%s", monitor_port)
        else:
            logger.warning("Web monitor could not be started automatically")
            logger.info("You can start it manually with: python -m laneswap.examples.start_monitor --api-url %s", api_url)
...
def main():
    ...
    parser.add_argument("--monitor-port", type=
        int, default=8080, help="Port to run the web monitor on")
